...
float4 sum(const float4 f1, const float4 f2) {
  float4 r;
  r.x = f1.x + f2.x;
  r.y = f1.y + f2.y;
  r.z = f1.z + f2.z;
...
struct Approximate {
  ...
  inline float4 operator()(Cell &c) {
    if (c.IsLeaf()) {
      ...
      float4 center =
          TapasBH::MapReduce(*this, c.subcells(),
                             center, sum);
      center.x /= center.w;
      center.y /= center.w;
      center.z /= center.w;
...
f4vec calc(f4vec &source) {
  ...
  // This can use MapReduce as well, but it isn't necessary here
  //TapasBH::Map(Approximate(), *root); // or, simply: approximate(*root);

...
#define create_task0(statement)			  __tg__.run_([=] { statement; }, __FILE__, __LINE__)
#define create_task1(s0,statement)		__tg__.run_([=,&s0] { statement; }, __FILE__, __LINE__)
#define create_task2(s0,s1,statement) __tg__.run_([=,&s0,&s1] { statement; }, __FILE__, __LINE__)
#define create_taskc(callable)        __tg__.run_(callable, __FILE__, __LINE__)
#define create_taskA(statement)			  __tg__.run_([&] { statement; }, __FILE__, __LINE__)
#define call_task(statement)			    do { statement; } while(0)
#define call_taskc(callable)			    callable()

#if create_task_and_wait_creates_task
#define create_task_and_wait(statement)   do { create_taskA(statement); wait_tasks; } while(0)
#define create_taskc_and_wait(callable)   do { create_taskc(callable); wait_tasks; } while(0)
#else
#define create_task_and_wait(statement)   do { call_task(statement); wait_tasks; } while(0)
#define create_taskc_and_wait(callable)   do { call_taskc(callable); wait_tasks; } while(0)
#endif

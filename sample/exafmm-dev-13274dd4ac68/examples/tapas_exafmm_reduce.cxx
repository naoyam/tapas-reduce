...
struct FMM_UpwardLeaf {
  template<class Cell, class Body>
  inline void operator()(Cell &c, Body &b) {
    ...
    for (int n=0; n<P; n++) {
      for (int m=0; m<=n; m++) {
        int nm  = n * n + n - m;
        int nms = n * (n + 1) / 2 + m;
        tapas::Reduce(c.attr().M[nms], b.SRC * Ynm[nm], sum<float>);
      }
    }
  }
};
...
struct FMM_Upward {
  template<class Cell>
  inline void operator()(Cell &p, Cell &c, real_t theta) {
    ...
    if (c.IsLeaf()) {
      //P2M(c);
      TapasFMM::Map(FMM_UpwardLeaf(), c.bodies());
    } else {
      TapasFMM::Map(*this, c.subcells(), theta);
      complex_t Ynm[P*P], YnmTheta[P*P];
      vec3 dX = tovec(p.center() - c.center());
        ...
        for (int k=0; k<=j; k++) {
          ...
          for (int n=0; n<=j; n++) {
            for (int m=std::max(-n,-j+k+n); m<=std::min(k-1,n); m++) {
              ...
              int nm    = n * n + n - m;
              M += c.attr().M[jnkms] * Ynm[nm] * real_t(IPOW2N(m) * ODDEVEN(n));
            }
            for (int m=k; m<=std::min(n,j+k-n); m++) {
              int jnkms = (j - n) * (j - n + 1) / 2 - k + m;
              int nm    = n * n + n - m;
              M += std::conj(c.attr().M[jnkms]) * Ynm[nm] * real_t(ODDEVEN(k+n+m));
            }
          }
          tapas::Reduce(p.attr().M[jks], M, sum<complex_t>);
        }
      }
    }
